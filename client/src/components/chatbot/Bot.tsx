...
const Bot: React.FC = () => {
  ...
  useEffect(() => {
    const initialMessageText =
      "Hello, I'm Helper, your student support companion. I'm here to listen and can offer some simple coping strategies if you're feeling stressed or overwhelmed. You can talk to me in English, Hindi, Punjabi, or Haryanvi.\n\nWhat's on your mind today?";
    const initialBotMessage: Message = {
      sender: "bot",
      text: initialMessageText,
    };
    const initialHistory: ChatContent = {
      role: "model",
  ...
  const parseMarkdown = (text: string): string => {
    ...
    return text
      ...
      .replace(
        urlRegex,
        '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-indigo-500 underline hover:text-indigo-700">$1</a>'
      );
  };

  const getBotResponse = async (
    prompt: string,
    currentChatHistory: ChatContent[]
  ) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (botMessageText) {
        ...
        const newBotHistory: ChatContent = {
          role: "model",
          parts: [{ text: botMessageText }],
        };
        setMessages((prev) => [...prev, newBotMessage]);
        setChatHistory((prev) => [...prev, newBotHistory]);
      } else {
        const errorMsg =
          "I'm sorry, I'm having a little trouble thinking right now. Could you please try again?";
        setMessages((prev) => [...prev, { sender: "bot", text: errorMsg }]);
      }
    } catch (error) {
      console.error("Error fetching bot response:", error);
      const errorMsg =
        "I seem to be having connection issues. Please check your internet and try again.";
      setMessages((prev) => [...prev, { sender: "bot", text: errorMsg }]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (userMessageText) {
      ...
      const newUserHistory: ChatContent = {
        role: "user",
  ...
  const handleClearChat = () => {
    const initialMessageText =
      "Hello, I'm Helper. I'm here to listen again whenever you need it.";
    const initialBotMessage: Message = {
      sender: "bot",
      text: initialMessageText,
    };
      ...
      <div className="w-full max-w-2xl h-[90vh] md:h-[80vh] flex flex-col bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl rounded-2xl shadow-2xl shadow-indigo-200/50 dark:shadow-sky-900/50 mx-4 my-4 border border-white/50 dark:border-slate-700/50">
        <header className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white p-4 rounded-t-2xl shadow-lg shadow-teal-500/20 flex items-center justify-between transition-colors">
          ...
          <div className="flex items-center space-x-1">
            ...
            >
              ...
              >
                <path d="M12 3a6.364 6.364 0 0 0 0 18 6.364 6.364 0 0 0 0-18Z" />
                <path d="M12 9v6" />
                <path d="M15 10.5h-6" />
              </svg>
            </button>
        ...
        {showDisclaimer && (
          <div className="p-3 bg-amber-50 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 text-xs text-center border-b border-amber-200/50 dark:border-amber-500/20 transition-colors">
            <p>
              <strong>Important:</strong> I am an AI, not a medical
              professional. If you are in crisis, please seek immediate help.
              You can call the{" "}
              <strong>KIRAN Mental Health Helpline at 1800-599-0019</strong>{" "}
              (24/7, free).
              <button
                onClick={() => setShowDisclaimer(false)}
                className="font-bold ml-2 p-1 rounded-full hover:bg-amber-200/50 dark:hover:bg-amber-800/50 transition-colors"
              >
                [X]
              </button>
            </p>
        ...
        <div
          ...
          className="flex-1 p-6 overflow-y-auto chat-container"
        >
        ...
        <form
          ...
          className="p-4 border-t border-slate-200/50 dark:border-slate-700/50 bg-white/50 dark:bg-slate-800/40 rounded-b-2xl transition-colors"
        >
          <div className="flex items-center bg-white dark:bg-slate-700 rounded-full border border-slate-200/80 dark:border-slate-600 focus-within:ring-2 focus-within:ring-indigo-400 transition-all duration-300 shadow-sm">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="p-2 text-indigo-500 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-slate-600 transition-all duration-300 rounded-full mr-2 disabled:opacity-50 disabled:hover:bg-transparent"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
